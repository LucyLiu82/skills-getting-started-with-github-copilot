  ...
  async function fetchActivities() {
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        // Create participant list items
        let participantItems = details.participants.map(participant => `
          <li>
            <span class="participant-email">${participant}</span>
            <button class="delete-btn" title="Unregister" data-activity="${name}" data-email="${participant}">&#x1F5D1;</button>
          </li>`).join("");
        if (participantItems === "") {
          participantItems = `<li class="no-participants">No participants yet</li>`;
        }
